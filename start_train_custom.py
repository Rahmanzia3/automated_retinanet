...
if __name__ == "__main__":
    ...
    data_gen_csv = convert.main(data_folder,project_dir)
    names_gen_csv = create_names_csv(text_dir_is_array[0],project_dir)

    backup_dir = os.path.join(project_dir,'back_up_'+opt.backbone)

    store_path = ' --snapshot-path ' + str(project_dir)
    custom = ' --backbone '+str(opt.backbone) + ' --epochs ' +str(opt.epochs)+ ' --steps '+str(opt.steps)+' --batch-size '+str(opt.batch_size)+ store_path
    run = 'keras_retinanet/bin/train.py '+str(custom)+' csv ' +str(data_gen_csv)+ ' ' +str(names_gen_csv)
